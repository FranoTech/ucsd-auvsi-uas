...
Callback::Callback( OpenGLForm::COpenGL ^ openglRef )
{
	IDeckLinkIterator * deckLinkIterator = NULL;
	IDeckLink *	deckLink = NULL;
	HRESULT result;

	m_callback = new DecklinkCallback( openglRef );	
	...
	{			
		...
		else // have input thingy
		{
			...
			config->SetVideoInputFormat( bmdVideoConnectionComposite );
...
DecklinkCallback::VideoInputFrameArrived( IDeckLinkVideoInputFrame *videoFrame, IDeckLinkAudioInputPacket *audioPacket)
{
	...
	if( firstFrame )
	{
		openGLPointer->CallbackSetup( frameWidth, frameHeight );
	}
	...
	for( int row=0; row < frameHeight; row += 2 ) 
	{
		...
		//char * ptr = &openGLPointer->buffer[row * frameWidth * 3 / 2];
...
/*void
DecklinkCallback::SaliencyThreadProcess( float *buffer) { //  int frameWidth, int frameHeight 
	Auvsi_Saliency * theSaliency = new Auvsi_Saliency();
	System::Diagnostics::Trace::WriteLine("Saliency created");
	DWORD theTime, otherTime, difference;

	// load image
	theTime = GetTickCount();

	pin_ptr<float> pinnedBuffer = buffer;

	float * pp = pinnedBuffer;

	theSaliency->loadImage( frameWidth, frameHeight/2, pp  );
	otherTime = GetTickCount();
	difference = otherTime - theTime;
	System::Diagnostics::Trace::WriteLine("Saliency loaded. Time to load image: " + difference + " (ms)");

	// compute image
	theTime = GetTickCount();
	theSaliency->computeSaliencyQuick();
	otherTime = GetTickCount();
	difference = otherTime - theTime;
	System::Diagnostics::Trace::WriteLine( "Saliency computed. Time to compute image: " + difference + " (ms)");
	delete theSaliency;
	isComputing = false;
